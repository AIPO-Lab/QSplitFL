...
print("="*80)
print("CAPABILITY-AWARE Q-LEARNING FOR SPLIT POINT OPTIMIZATION (CIFAR-100)")
print("="*80)
print(f"\nConfiguration:")
...
print("\n Loading and preparing CIFAR-100 dataset...")
...
cifar100_dataset = torchvision.datasets.CIFAR100(root="./data", train=True, download=True, transform=transform_train)
cifar100_test_dataset = torchvision.datasets.CIFAR100(root='./data', train=False, download=True, transform=transform_test)

# Create heterogeneous data distribution
total_samples = len(cifar100_dataset)
indices = np.arange(len(cifar100_dataset))
...
for i in range(num_clients):
    ...
    subset = Subset(cifar100_dataset, subindices)
...
model_instance, model_display_name = get_model_by_choice(
    model_choice, 100, input_channels=3
)

# Initialize server with selected model
server = Server(100, model_instance, cifar100_test_dataset)
env = FL_Environment(num_clients, global_class_dist=np.ones(100) / 100)
...
for episode in range(num_episodes):
    ...
    for round_num in range(num_rounds):
        ...
        # Update client capabilities every 5 rounds (not every round)
        if (round_num + 1) % 5 == 0:
            env.update_client_capabilities()
            print(f"  [Capability Check] Capabilities updated after round {round_num + 1}")
        ...
        # Display detailed metrics with enhanced per-client information
        print(f"\n{'='*70}")
        print(f"ROUND {round_num + 1} RESULTS")
        print(f"{'='*70}")
        print(f"Global (Avg Client): Acc={new_acc:.4f}, Loss={new_loss:.4f}")
        print(f"\nPer-Client Metrics:")
        print(f"{'Client ID':<12} {'Loss':<12} {'Accuracy':<12}")
        print(f"{'-'*36}")
        for idx, (client_id, loss, acc) in enumerate(zip(selected_client_idxs, client_losses, client_accuracies)):
            print(f"Client {client_id:<6} {loss:<12.4f} {acc:<12.4f}")
        print(f"{'-'*36}")
        print(f"Average:     {avg_client_loss:<12.4f} {avg_client_accuracy:<12.4f}")
        print(f"{'='*70}\n")
...
axes[0, 0].set_title('Test Accuracy Over Time (CIFAR-100)')
...
axes[0, 1].set_title('Test Loss Over Time (CIFAR-100)')
...
axes[1, 0].set_title('Reward Over Time (CIFAR-100)')
...
axes[1, 1].set_title('Selected Split Points (CIFAR-100)')
axes[1, 1].set_title('Selected Split Points')
...
axes[2, 0].set_title('Exploration Rate Decay (CIFAR-100)')
...
plt.tight_layout()
plt.savefig('./complete_rl_results_cifar100.png', dpi=150, bbox_inches='tight')
print(" Results saved to 'complete_rl_results_cifar100.png'")
