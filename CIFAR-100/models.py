"""
Neural Network Models for Federated Split Learning - CIFAR-100

This module contains all available CNN architectures:
1. CNN - ResNetFed (original/baseline)
...
Input: 3 channels, Output: 100 classes
"""
...
# ============================================================================
# Original CNN Architecture for CIFAR-100
# ============================================================================

class ResNetFed(nn.Module):
    """
    ResNet-18 variant optimized for CIFAR-100 (32x32 images).
    ...
    def _forward_client(self, x):
        """
        Client-side forward pass up to split point (Layers 5-9).
        
        Mapping logic (ResNet18-CIFAR100):
        - 5: Conv1 + Layer1 (3blks) -> After Layer 1
        - 6: Layer 2 Block 0
        - 7: Layer 2 Block 1 (After Layer 2)
        - 8: Layer 3 Block 0
        - 9: Layer 3 Block 1 (After Layer 3)
        """
        # Always run initial block and layer1
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        
        if self.split_layer <= 5:
            # Check if we need to cut early within layer1? 
            # Requirement says "Layers 5-9". 5 usually means after Layer 1.
            return out
            
        # Layer 2 (2 blocks for ResNet18)
        # Split 6: After L2B0
        if self.split_layer == 6:
            out = self.layer2[0](out)
            return out
            
        # Split 7: After L2B1 (End of Layer 2)
        out = self.layer2(out)
        ...
        # Split 9: After L3B1 (End of Layer 3)
        out = self.layer3(out)
        if self.split_layer == 9:
            return out
            
        # If > 9, client runs more (but strict range is 5-9)
        out = self.layer4(out)
        return out
...
class CNNModel(nn.Module):
    """
    Alternative CNN architecture for CIFAR-100.
...
class SimpleNN(nn.Module):
    ...
    Can be used for both MNIST and CIFAR-100 with appropriate input size.
    """
    def __init__(self, input_size=3072, num_classes=100):
...
def get_model_by_choice(choice, num_classes, input_channels):
    ...
    if choice == 1:
        # CNN (baseline)
        model = ResNetFed(num_classes=num_classes)
        return model, "CNN"
    elif choice == 2:
        # ResNet50
...
class ResNet50(nn.Module):
    ...
    def _forward_client(self, x):
        ...
        if self.split_layer <= 40:
            blocks_to_run = (self.split_layer - 22 + 2) // 3 
            blocks_to_run = max(1, min(blocks_to_run, 6))
