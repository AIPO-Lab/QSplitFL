...
mnist_test_dataset = torchvision.datasets.MNIST(root='./data', train=False, download=True, transform=transform_test)
...
server = Server(10, model_instance, mnist_test_dataset)
...
results_df.to_csv('mnist_rl_results.csv', index=False)
print(" Results saved to 'mnist_rl_results.csv'")

# Plot 1: Accuracy
axes[0, 0].plot(rounds_axis, all_accuracies, 'b-', alpha=0.7, label='Test Accuracy')
axes[0, 0].set_xlabel('Round')
axes[0, 0].set_ylabel('Accuracy')
axes[0, 0].set_title('Test Accuracy Over Time (MNIST)')
...
axes[0, 1].set_title('Test Loss Over Time (MNIST)')
...
axes[1, 0].set_title('Reward Over Time (MNIST)')
...
axes[1, 1].set_title('Selected Split Points (MNIST)')
...
axes[2, 0].set_title('Exploration Rate Decay (MNIST)')
...
plt.tight_layout()
plt.savefig('./complete_rl_results_mnist.png', dpi=150, bbox_inches='tight')
print(" Results saved to 'complete_rl_results_mnist.png'")
