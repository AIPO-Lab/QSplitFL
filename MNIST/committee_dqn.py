...
class CommitteeDQN:
    ...
    def __init__(self, committee_size=3, state_size=6, action_size=5, total_layers=10,
        ...
        self.min_split = int(np.ceil(total_layers / 2))  # Paper: ⌈L/2⌉
    ...
    def select_action(self, state, use_epsilon_greedy=True):
        ...
        # Create wrapper objects for voting (with get_q_value method)
        # These compute real Q-values from the actual DQN heads
        class CommitteeMemberWrapper:
            ...
            def get_q_value(self, state, action):
                """Compute real Q-value using actual DQN head and encoder."""
                state_tensor = torch.FloatTensor(state).unsqueeze(0)
                action_idx = action - self.min_split
                with torch.no_grad():
        ...
        committee_members = [
            CommitteeMemberWrapper(self.dqn_heads[m], self.shared_encoder, self.min_split)
            for m in range(self.committee_size)
        ]
        
        selected_action, vote_counts, is_tie = majority_vote(
            proposals, state, committee_members, action_space
        )
